"""
UK Flooring Direct - Shopify Product Scraper
Site: https://www.ukflooringdirect.co.uk
Method: Shopify /products.json API (no auth required)

Fields extracted:
  - Name, Description, Price, Images, SKU, Category, Stock Status

Output: products_ukflooring.csv / .json / .xlsx
"""
...
def fetch_all_products() -> list[dict]:
    ...
    print(f"[UKFD] Starting fetch from {API_URL}")
    ...
    while True:
        ...
        try:
            ...
            print(f"[UKFD] Request error on page {page_num}: {e}")
            break

        data = resp.json().get("products", [])
        if not data:
            print(f"[UKFD] No more products at page {page_num}. Done.")
            break
        ...
        print(f"[UKFD] Page {page_num}: fetched {len(data)} products "
              f"(total rows so far: {len(products)})")

        if not next_page_info:
            print("[UKFD] No next page link. Finished pagination.")
            break
...
def parse_product(product: dict) -> list[dict]:
    ...
    for variant in variants:
        ...
        # UK Flooring specific: extract m2 price hint from tags if present
        m2_price_tag = ""
        for tag in product.get("tags", []):
            if "per-m2" in tag.lower() or "perm2" in tag.lower() or "per m2" in tag.lower():
                m2_price_tag = tag
                break
...
def save_csv(products: list[dict], filepath: str):
    ...
    print(f"[UKFD] CSV saved  → {filepath}  ({len(products)} rows)")


def save_json(products: list[dict], filepath: str):
    with open(filepath, "w", encoding="utf-8") as f:
        json.dump(products, f, ensure_ascii=False, indent=2)
    print(f"[UKFD] JSON saved → {filepath}  ({len(products)} records)")
...
def save_xlsx(products: list[dict], filepath: str):
    ...
    # Header style
    header_fill = PatternFill("solid", fgColor="1E5631")  # Green theme for flooring
    ...
    wb.save(filepath)
    print(f"[UKFD] XLSX saved → {filepath}  ({len(products)} rows)")
...
def main():
    ...
    if not products:
        print("[UKFD] No products found. Check the site or your network.")
        return

    print(f"\n[UKFD] Total rows collected: {len(products)}")

    save_csv(products,  f"{base_name}.csv")
    save_json(products, f"{base_name}.json")
    save_xlsx(products, f"{base_name}.xlsx")

    print(f"\n[UKFD] ✅ Done! Files saved in '{OUTPUT_DIR}/' folder.")
